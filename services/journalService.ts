...
const path = (root:string, name:string) => `${root}/${enc(name)}`;
...
async function ensureDir(uri:string, name:string) { const target=path(uri,name); try { await SAF.readDirectoryAsync(target); return target; } catch { return SAF.makeDirectoryAsync(uri,name); } }
async function dailyUri(root:string, date:string) { const [year,month,day]=date.split('-'); const journal=await ensureDir(root,'Journal'); const yr=await ensureDir(journal,year); const mo=await ensureDir(yr,month); const name=`${day}.md`; const uri=path(mo,name); try { await SAF.readAsStringAsync(uri); } catch { await SAF.createFileAsync(mo,name,'text/markdown'); await SAF.writeAsStringAsync(uri,header(date)); } return uri; }
...
async function walk(uri:string, dates:string[]):Promise<void> { const names=await SAF.readDirectoryAsync(uri); for(const n of names) { const u=path(uri,n); if(n.endsWith('.md')) dates.push(u); else { try { await walk(u,dates); } catch { /* file */ } } } }
export async function loadEntries(root:string):Promise<JournalEntry[]> { const journal=path(root,'Journal'); const files:string[]=[]; try { await walk(journal,files); } catch { return []; } const all:JournalEntry[]=[]; for(const uri of files) { const parts=decodeURIComponent(uri).split('/'); const date=`${parts.at(-3)}-${parts.at(-2)}-${parts.at(-1)!.replace('.md','')}`; try { all.push(...parseJournal(await read(uri),date)); } catch { /* preserve unreadable files */ } } return all.sort((a,b)=>b.createdAt.localeCompare(a.createdAt)); }
export async function initializeJournal(root:string) { await ensureDir(root,'Journal'); await ensureDir(root,'Attachments'); const settings=path(root,'mindlog-settings.json'); try { await read(settings); } catch { await SAF.createFileAsync(root,'mindlog-settings.json','application/json'); await SAF.writeAsStringAsync(settings,JSON.stringify({format:'mindlog-v1'},null,2)); } }
